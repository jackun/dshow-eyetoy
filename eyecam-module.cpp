...
STDMETHODIMP VCamFactory::CreateInstance(LPUNKNOWN parent, REFIID, void **p_ptr)
{
	...
	*p_ptr = nullptr;
	...
	if (IsEqualCLSID(cls, CLSID_EyeCam_Video)) {
...
static bool RegServers(bool reg)
{
	...
	if (reg) {
		return RegServer(CLSID_EyeCam_Video, L"EyeCam",
				 file);
	} else {
		return UnregServer(CLSID_EyeCam_Video);
	}
}
...
static bool RegFilters(bool reg)
{
	...
	if (reg) {
		...
		hr = fm->RegisterFilter(CLSID_EyeCam_Video,
					L"EyeCam", &moniker,
